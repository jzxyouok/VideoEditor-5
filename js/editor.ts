...
var Default_Project_Name: string = "Projet_1";
var tlObj = {},
    libObj = [],
    optionsFormSelector = $('#optionsForm'),
...
var sequence = {
    fillTlContent: function (tl) {
        ...
        $(tl).find('.rushElement').each(function () {
            ...
            tlObj[currentTLID][uid]=<ElementOnTl> {
                name: currentRushId,
                uid: uid,
                src: currentRushSrc,
...
var scope = {
    actualize: function (play?: string) {
        ...
        var scopeCurTime = $('#scopeCurTime'),
            ...
            totalHours = Math.floor(totalTime.seconds/3600),
            totalMinutes = Math.floor((totalTime.seconds%3600)/60),
            totalSeconds = Math.floor(totalTime.seconds%60),
            totalImages = Math.floor(totalTime.images%24),
            currentHours = Math.floor(currentTime.seconds/3600),
            currentMinutes = Math.floor((currentTime.seconds%3600)/60),
            currentSeconds = Math.floor(currentTime.seconds%60),
            currentImages = Math.floor(currentTime.images%24);
        ...
        $.each(rushToShow, function (key) {
            ...
            if(scopePlayer.find('.'+key).length >= 1){
                if(type == "mp4"){
                    var vid = <HTMLVideoElement>scopePlayer.find('.'+key)[0].querySelector("video");
                    ...
                    var aud = <HTMLAudioElement>scopePlayer.find('.'+key)[0].querySelector("audio");
        ...
        $.each(rushToShow, function (key) {
            ...
            if (scopePlayer.find('.' + key).length >= 1) {
                if (type == "mp4" || type == "mp3") {
                    var vid = <HTMLVideoElement> scopePlayer.find('.' + key).find('.imgInScope')[0];
    ...
    applyDropEvents: function (dropper) {
        ...
        dropper.addEventListener('drop', function (e) {
            ...
            if (filesLen) {
                ...
                var tlPos: number = $(target.parentNode).offset().left;// position absolue de la TL
                var tarPos: number = $(dnd.draggedElement).offset().left;// position absolue de l'element au drag
                var moInTarPos:number = dnd.onDragMousePos.left - tarPos;// position de la souris dans l'element

                var mousePos:number = e.clientX;// position absolue de la souris au drop
                var moInTlPos:number = (mousePos - tlPos) + target.parentNode.scrollLeft;// position de la souris dans la TL au drop
                var tarInTLPosDrop:number = moInTlPos-moInTarPos; //position de l'element dans la TL au drop
                ...
                if (type == "mp4") {
                    tmpUidSelector.on('mousedown', function () {
                        ...
                        document.querySelector(".thumb").addEventListener('loadedmetadata', function () {
                            ...
                            var vid = <HTMLVideoElement>document.querySelector(".thumb");
                    ...
                    tmpUidSelector.on('mousedown', function () {
                        ...
                        $('#miniScopePlayer').html('<audio class="thumb" controls preload="metadata">' +
                            ...
                            '</audio>');
                        document.querySelector(".thumb").addEventListener('loadedmetadata', function () {
                            var aud = <HTMLAudioElement>document.querySelector(".thumb");
